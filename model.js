...
export class PokemonTeam {
  ...
  addPokemon(pokemon) {
    ...
    const oldPokemon = this.selectedTeam.find((oldPokemon) => oldPokemon.name === pokemon.name) || null;
    if (!oldPokemon) {
  ...
  decreaseSpecialPower(winnerName, pointsToDecrease) {
    const winner = this.selectedTeam.find((pokemon) => pokemon.name === winnerName);
...
export class Player {
  ...
  attack(target) {
      ...
      if (attackingPokemon && defendingPokemon) {
          console.log(`${this.name}'s ${attackingPokemon.name} attacks ${target.getName()}'s ${defendingPokemon.name}!`);
          attackingPokemon.attack(defendingPokemon);

          if (defendingPokemon.isFainted()) {
              console.log(`${target.getName()}'s ${defendingPokemon.name} has fainted!`);
